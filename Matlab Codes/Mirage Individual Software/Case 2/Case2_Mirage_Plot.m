...
%% Case Setup 
disp('<strong>input variables for (n^2)(y) = n0^2+(np^2)(1-e^(-a*y)) : </strong>');
a=2.6*10^-2;
np=1.5;
H=5;
...
nH=sqrt(n0^2+np^2*(1.0-exp(-a*H))); %% Formula based on x
%% Change of y
fprintf('\n')
N=10; %% Number Array
...
Y = 0:1:H;
nY = sqrt(n0^2+np^2*(1.0-exp(-a*Y))); %% Case drawing based on y
